...
describe('JsonFileStorage', () => {
  ...
  describe('built-in scenarios', () => {
    let builtInStorage: JsonFileStorage;

    beforeEach(() => {
      // Seed a built-in scenario file
      mockFsData.state.files.set(
        '/built-in/scenario-bi.json',
        JSON.stringify(makeScenario({ id: 'scenario-bi', name: 'Built-in', builtIn: true })),
      );
      builtInStorage = new JsonFileStorage('/test-base', mockFsData.fs, '/built-in');
    });

    it('returns built-in scenario by ID', async () => {
      const result = await builtInStorage.getScenario('scenario-bi');
      expect(result?.id).toBe('scenario-bi');
      expect(result?.name).toBe('Built-in');
    });

    it('falls back to custom scenario when not built-in', async () => {
      await builtInStorage.saveScenario(makeScenario({ id: 'custom-1', name: 'Custom' }));
      const result = await builtInStorage.getScenario('custom-1');
      expect(result?.name).toBe('Custom');
    });

    it('lists both built-in and custom scenarios', async () => {
      await builtInStorage.saveScenario(makeScenario({ id: 'custom-1' }));
      const all = await builtInStorage.listScenarios();
      expect(all.length).toBeGreaterThanOrEqual(2);
    });

    it('deduplicates custom scenarios that shadow built-in IDs', async () => {
      // Manually place a custom scenario file on disk with the same ID as a built-in
      mockFsData.state.files.set(
        '/test-base/scenarios/custom/scenario-bi.json',
        JSON.stringify(makeScenario({ id: 'scenario-bi', name: 'Shadow' })),
      );
      const all = await builtInStorage.listScenarios();
      const matches = all.filter((s) => s.id === 'scenario-bi');
      expect(matches).toHaveLength(1);
      expect(matches[0].name).toBe('Built-in'); // built-in takes precedence
    });

    it('throws when saving a built-in scenario', async () => {
      await expect(
        builtInStorage.saveScenario(makeScenario({ id: 'scenario-bi', name: 'Modified' })),
      ).rejects.toThrow('Cannot modify a built-in scenario');
    });

    it('throws when deleting a built-in scenario', async () => {
      await expect(
        builtInStorage.deleteScenario('scenario-bi'),
      ).rejects.toThrow('Cannot delete a built-in scenario');
    });

    it('caches built-in scenarios after first load', async () => {
      await builtInStorage.listScenarios();
      await builtInStorage.listScenarios();
      // readdir should only be called for the built-in dir once (plus init calls)
      const readdirCalls = (mockFsData.fs.readdir as ReturnType<typeof vi.fn>).mock.calls;
      const builtInReaddirCalls = readdirCalls.filter(
        (c: string[]) => c[0] === '/built-in',
      );
      expect(builtInReaddirCalls).toHaveLength(1);
    });

    it('handles readdir failure for built-in dir gracefully', async () => {
      const failFs = createMockFs();
      (failFs.fs.readdir as ReturnType<typeof vi.fn>).mockImplementation((dirPath: string) => {
        if (dirPath === '/built-in-bad') return Promise.reject(new Error('ENOENT'));
        return Promise.resolve([]);
      });
      const failStorage = new JsonFileStorage('/test-base', failFs.fs, '/built-in-bad');
      const scenarios = await failStorage.listScenarios();
      expect(scenarios).toEqual([]);
    });

    it('skips corrupt built-in scenario files', async () => {
      mockFsData.state.files.set('/built-in/bad.json', '{corrupt');
      const all = await builtInStorage.listScenarios();
      // Should still include the valid built-in scenario
      expect(all.some((s) => s.id === 'scenario-bi')).toBe(true);
    });

    it('skips non-json files in built-in dir', async () => {
      mockFsData.state.files.set('/built-in/readme.txt', 'not a scenario');
      const all = await builtInStorage.listScenarios();
      expect(all.some((s) => s.id === 'scenario-bi')).toBe(true);
    });

    it('returns empty built-in cache when builtInScenariosDir is undefined', async () => {
      // The default storage (no builtInScenariosDir) should have no built-in scenarios
      const all = await storage.listScenarios();
      expect(all).toEqual([]);
    });

    it('filters built-in scenarios by category', async () => {
      mockFsData.state.files.set(
        '/built-in/scenario-bi2.json',
        JSON.stringify(makeScenario({ id: 'scenario-bi2', category: 'reasoning', builtIn: true })),
      );
      // Clear cache so the new file is picked up
      const freshStorage = new JsonFileStorage('/test-base', mockFsData.fs, '/built-in');
      const filtered = await freshStorage.listScenarios({ category: 'reasoning' });
      expect(filtered.some((s) => s.id === 'scenario-bi2')).toBe(true);
      expect(filtered.some((s) => s.id === 'scenario-bi')).toBe(false);
    });
  });
});
